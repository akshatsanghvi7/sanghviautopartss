...
const purchaseFormSchema = z.object({
  ...
  items: z.array(purchaseItemSchema).min(1, "At least one item must be added"),
...
export function PurchaseFormDialog({ isOpen, onOpenChange, onSubmit, inventoryParts, inventorySuppliers }: PurchaseFormDialogProps) {
  ...
  const { control, register, handleSubmit, reset, watch, setValue, getValues, formState: { errors } } = useForm<PurchaseFormData>({
    resolver: zodResolver(purchaseFormSchema),
    defaultValues: { purchaseDate: new Date(), items: [], shippingCosts: 0, otherCharges: 0, paymentType: 'bank_transfer', status: 'Pending', supplierId: undefined, supplierName: '', supplierContactPerson: '', supplierEmail: '', supplierPhone: '', supplierInvoiceNumber: '', notes: '' },
  });
  ...
  const resetFormState = useCallback(() => {
    reset({ purchaseDate: new Date(), items: [], shippingCosts: 0, otherCharges: 0, paymentType: 'bank_transfer', status: 'Pending', supplierId: undefined, supplierName: '', supplierContactPerson: '', supplierEmail: '', supplierPhone: '', supplierInvoiceNumber: '', notes: '' });
    setPartSearchTerm(''); setCurrentQuantityInput(1); setCurrentUnitCostInput(0); setSelectedPartForAdding(null); setSupplierSuggestions([]); setIsSupplierPopoverOpen(false);
  }, [reset]);

  useEffect(() => { if (!isOpen) resetFormState(); else if (!getValues('purchaseDate')) setValue('purchaseDate', new Date()); }, [isOpen, getValues, setValue, resetFormState]);

  const handleSupplierNameChange = (name: string) => {
    setValue('supplierName', name, { shouldValidate: true }); 
    if (name.length > 0) {
      const filtered = inventorySuppliers.filter(s => s.name.toLowerCase().includes(name.toLowerCase()));
      setSupplierSuggestions(filtered);
      setIsSupplierPopoverOpen(filtered.length > 0 && document.activeElement === document.getElementById('supplierNameInput'));
    } else {
      setSupplierSuggestions([]); setIsSupplierPopoverOpen(false);
      setValue("supplierId", undefined); setValue("supplierContactPerson", ""); setValue("supplierEmail", ""); setValue("supplierPhone", "");
    }
  };

  const handleSupplierSelect = (supplier: Supplier) => {
    setValue("supplierId", supplier.id); setValue("supplierName", supplier.name); 
    setValue("supplierContactPerson", supplier.contactPerson || ""); setValue("supplierEmail", supplier.email || ""); setValue("supplierPhone", supplier.phone || "");
    setSupplierSuggestions([]); setIsSupplierPopoverOpen(false);
    document.getElementById('supplierContactPerson')?.focus();
  };

  const handlePartSearch = (term: string) => { setPartSearchTerm(term); if (!term) setSelectedPartForAdding(null); };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) resetFormState(); onOpenChange(open); }}>
      <DialogContent className="sm:max-w-2xl md:max-w-3xl lg:max-w-4xl flex flex-col max-h-[90vh]">
        <DialogHeader><DialogTitle>Create New Purchase Order</DialogTitle><DialogDescription>Fill in details for the new PO.</DialogDescription></DialogHeader>
        <form id="purchaseFormId" onSubmit={handleSubmit(handleFormSubmitInternal)} className="flex-1 space-y-6 overflow-y-auto p-4 custom-scrollbar">
          <div className="space-y-4 p-4 border rounded-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div><Label htmlFor="supplierContactPerson">Contact Person</Label><Input id="supplierContactPerson" {...register("supplierContactPerson")} /></div>
              <div><Label htmlFor="supplierEmail">Email</Label><Input id="supplierEmail" type="email" {...register("supplierEmail")} />{errors.supplierEmail && <p className="text-sm text-destructive">{errors.supplierEmail.message}</p>}</div>
              <div><Label htmlFor="supplierPhone">Phone</Label><Input id="supplierPhone" type="tel" {...register("supplierPhone")} /></div>
            </div>
          </div>
          <div className="space-y-4 p-4 border rounded-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
            <div><Label htmlFor="shippingCosts">Shipping Costs (₹)</Label><Input id="shippingCosts" type="number" {...register("shippingCosts")} placeholder="0.00" step="0.01" /></div>
            <div><Label htmlFor="otherCharges">Other Charges (₹)</Label><Input id="otherCharges" type="number" {...register("otherCharges")} placeholder="0.00" step="0.01" /></div>
            <div className="space-y-1 text-right md:col-start-3"><p className="text-muted-foreground">Subtotal: <span className="font-semibold text-foreground">₹{subTotal.toFixed(2)}</span></p>{numericShipping > 0 && <p className="text-muted-foreground">Shipping: <span className="font-semibold text-foreground">₹{numericShipping.toFixed(2)}</span></p>}{numericOtherCharges > 0 && <p className="text-muted-foreground">Other: <span className="font-semibold text-foreground">₹{numericOtherCharges.toFixed(2)}</span></p>}<p className="text-xl font-bold text-foreground">Net Amount: ₹{netAmount.toFixed(2)}</p></div>
          </div>
          <div className="pt-4"><Label htmlFor="notes">Notes</Label><Textarea id="notes" {...register("notes")} placeholder="Any specific notes for this PO..." /></div>
        </form>
        <DialogFooter className="pt-4 border-t bg-background pb-4"><DialogClose asChild><Button type="button" variant="outline" onClick={resetFormState}>Cancel</Button></DialogClose><Button type="submit" form="purchaseFormId">Create Purchase Order</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
